...
An ultra-high-performance environmental monitoring system featuring direct **ESP32** sensor acquisition (**DHT11** Digital Temperature/Humidity Sensor with time-aware AI diurnal classification), a **FastAPI Real-Time Backend** with sub-millisecond in-memory caching and 60fps **WebSocket & Server-Sent Events (SSE)** broadcasting, an OpenRouter Nemotron AI biometeorological intelligence engine, an interactive Web Dashboard, and a native **Android Application** built with Kotlin, Jetpack Compose, and Material 3.
...
│                                            │    ESP32 Node        ├─────────────────────>│  FastAPI Cloud Server  ├────────────────────────>│  Android App & Web UI  │
│                                            │ (Direct Acquisition) │  JSON Telemetry      │  (Real-Time Broadcast) │  (Zero-Lag Instant UI)  │  (Jetpack Compose)     │
│                                            └───────────┬──────────┘                      └───────────┬────────────┘                         └────────────────────────┘
│                                                        │                                             │
│                                                        ▼                                             ▼
│  ┌──────────────────────┐    GPIO 2 (Onboard)       ┌──────────────────┐                       ┌────────────────────────┐
...
| :--- | :--- | :--- |
| **Direct Sensor Node** | ESP32 (C++ / Arduino Core) | Direct digital DHT11 acquisition (`GPIO 4`) with bit-bang microsecond protocol decoder, sub-second fast Wi-Fi reconnect, 120s deep sleep, and high-frequency real-time HTTPS push. |
